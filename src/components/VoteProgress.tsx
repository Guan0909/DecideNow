          ...
          >
            <div
              className={`absolute inset-0 rounded-2xl transition-all duration-[1200ms] ease-out-expo ${
                isWinner ? "bg-primary/8" : "bg-primary/4"
              }`}
            ...
            <div className="relative flex items-center gap-4 p-5">
              <div className={`flex h-11 w-11 shrink-0 items-center justify-center rounded-xl text-sm font-bold transition-all duration-500 ${
                isWinner ? "bg-primary text-white shadow-md shadow-primary/20" : "bg-foreground/[0.04] text-foreground/50"
              }`}>
                {isWinner ? "👑" : `${pct}%`}
              </div>
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  ...
                  {isWinner && <span className="shrink-0 rounded-full bg-primary/10 px-2.5 py-0.5 text-[11px] font-semibold tracking-wide text-primary">🏆 胜出</span>}
